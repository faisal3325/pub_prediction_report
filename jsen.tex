...
\begin{abstract}
In this technical report we have implemented a big data analytics framework on a PlayerUnknown’s Battlegrounds (PUBG) dataset from Kaggle. We analysed a few points using the dataset and answered our research questions where we focused on the performance of players. Our research questions debunked wether playing solo or playing in a team is more better for performance and gaining points and perks in the game. Using 29 features, we engineered a couple more features to make our analytics stronger. We also performed a few prediction algorithms on the dataset to predict win place percentile of players. We then compared these algorithms and came to the conclusion of which is the best performing algorithm. Our framework is implemented on Apache Spark using Google Cloud Platform. We used BigQuery and Cloud Dataproc to implement the system. The source code of this report is hosted on Github and open sourced
\end{abstract}
...
$\_analysis.ipynb$ is our main program which is deployed on the cluster. $ml.ipynb$ is where we did some trail runs in our local system. $pre\-processing.ipynb$ is the notebook file where we did pre-processing of our data along with feature engineering. There is a $trails$ folder where we have some extra code in which we did experimentations on the data.
